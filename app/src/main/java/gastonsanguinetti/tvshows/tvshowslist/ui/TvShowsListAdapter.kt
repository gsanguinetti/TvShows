...
class TvShowsListAdapter(val onItemSelected: (tvShow: TvShow, posterView :ImageView) -> Unit)
    : PagedListAdapter<Result, RecyclerView.ViewHolder>(SHOW_COMPARATOR) {
    ...
    inner class ShowItemViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(tvShow: TvShow?) {
            tvShow!!.run {
                ...
                backdropPath?.let { showPosterImageView.loadImage(ImageUrlBuilder().build(it)) }
                showRatingBar.rating = voteAverage.toFloat() / 2f
                itemView.setOnClickListener { onItemSelected(this, showPosterImageView) }
            }
        }
    }
